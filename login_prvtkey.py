...
def login_screen(root):
    ...
    logo_label = tk.Label(root, image=logo_image, bg='#121B23')
    ...
    # Butonlar
    next_button = tk.Button(root, text="Next", font=("Arial", 12), bg='#61cbf4', fg='white', command=lambda: process_private_key(prvt_key_entry.get(), root))
    next_button.pack(fill='x', expand=True, pady=(10, 2))

    back_button = tk.Button(root, text="Back", font=("Arial", 12), bg='#61cbf4', fg='white', command=lambda: mainscreen.show_main_screen(root))
...
def process_private_key(private_key_hex, root):
    try:
        ...
        # Symphony adresini hesaplama
        symphony_address = get_symphony_address_from_private_key(private_key_hex)
        if symphony_address:
            # walletaction.py'daki wallet_actions fonksiyonunu çağırarak adres bilgilerini göster
            walletaction.wallet_actions(root, symphony_address, private_key_hex)
        else:
            messagebox.showerror("Error", "Invalid private key. Please check and try again.")
    except Exception as e:
        messagebox.showerror("Error", f"An error occurred: {str(e)}")

def get_symphony_address_from_private_key(private_key_hex):
    ...
    # Symphony adresi oluşturma
    symphony_address = bech32.bech32_encode('symphony', bech32.convertbits(ripemd160_bpk, 8, 5))
